...
use termcolor::{self, Color, ColorSpec};
...
pub async fn send_waiting_for_startup(
    remote_sup_addr: &ResolvedListenCtlAddr,
    msg: impl Clone + Into<SrvMessage> + fmt::Debug,
) -> Result<()> {
    let start = Instant::now();
    let should_wait_for_startup = should_wait_for_local_supervisor_startup(remote_sup_addr);
    let mut warned = false;

    loop {
        match send(remote_sup_addr, msg.clone()).await {
            Err(crate::error::Error::CtlClient(SrvClientError::ConnectionRefused))
                if should_wait_for_startup && start.elapsed() < SUPERVISOR_STARTUP_WAIT =>
            {
                if !warned {
                    let mut ui = UI::default_with_env();
                    let _ = ui.warn(
                        "The local Supervisor is not accepting commands yet; \
...
fn should_wait_for_local_supervisor_startup(remote_sup_addr: &ResolvedListenCtlAddr) -> bool {
    should_wait_for_local_supervisor_startup_with_lock_file(
        remote_sup_addr,
        supervisor_lock_file_path(),
    )
}

fn should_wait_for_local_supervisor_startup_with_lock_file<P>(
    remote_sup_addr: &ResolvedListenCtlAddr,
    lock_file_path: P,
) -> bool
where
...
fn handle_ctl_reply(reply: &SrvMessage) -> result::Result<(), SrvClientError> {
    ...
    match reply.message_id() {
        "ConsoleLine" => {
            let m = reply
                .parse::<sup_proto::ctl::ConsoleLine>()
        ...
        "NetProgress" => {
            let m = reply
                .parse::<sup_proto::ctl::NetProgress>()
                .map_err(SrvClientError::Decode)?;
...
mod tests {
    ...
    fn recent_supervisor_lock_file_is_detected() {
        ...
        assert!(!has_recent_supervisor_lock_file(
            &lock_file,
    ...
    fn startup_wait_is_only_for_local_addresses_during_the_wait_window() {
        ...
        assert!(should_wait_for_local_supervisor_startup_with_lock_file(
            &local, &lock_file
        ));
        assert!(!should_wait_for_local_supervisor_startup_with_lock_file(
            &remote, &lock_file
