...
pub fn start(ui: &mut UI, cache_path: &Path, analytics_path: &Path) -> Result<()> {
    ...
    try!(ui.para("While you can build and run Habitat packages without sharing them on the public \
    ...
    try!(ui.para("If you would like to share your packages on the depot, please enter your GitHub \
    ...
    try!(ui.para("The `hab` command-line tool will optionally send anonymous usage data to \
               ...
               below."));
    try!(ui.para("We collect this data to help improve Habitat's user experience. For example, we \
    ...
    try!(ui.para("To see what kinds of data are sent and how they are anonymized, please read more \
...
fn prompt_auth_token(ui: &mut UI) -> Result<String> {
    ...
    let default = match config.auth_token {
        Some(o) => {
            try!(ui.para("You already have a default auth token set up, but feel free to change it \
                       if you wish."));
            Some(o)
        }
