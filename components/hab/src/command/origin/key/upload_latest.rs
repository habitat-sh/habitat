...
pub fn start(ui: &mut UI,
    ...
    let public_keyfile = try!(SigKeyPair::get_public_key_path(&latest.name_with_rev(), cache));
    ...
    if with_secret {
        let secret_keyfile = try!(SigKeyPair::get_secret_key_path(&latest.name_with_rev(), cache));
