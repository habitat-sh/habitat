...
| `HAB_CTL_SECRET` | Supervisor | no default | Shared secret used for [communicating with a Supervisor]({{< relref "using_habitat/#remote-control" >}}). |
...
| `HAB_ORG` | Supervisor | no default | Organization to use when running with [service group encryption]({{< relref "using_habitat/#using-encryption" >}})
...
| `HAB_RING` | Supervisor | no default | The name of the ring used by the Supervisor when running with [wire encryption]({{< relref "using_habitat#using-encryption" >}}) |
| `HAB_RING_KEY` | Supervisor | no default | The contents of the ring key when running with [wire encryption]({{< relref "using_habitat#using-encryption" >}}). Useful when running in a container. |
...
| `HAB_UPDATE_STRATEGY_FREQUENCY_MS` | Supervisor | 60000 | Interval in milliseconds governing how often to check for service updates when running with an [update strategy]({{< relref "using_habitat#using-updates" >}}). Note: This variable has been deprecated. Users should instead utilize the [--service-update-period]({{< relref "habitat_cli/#hab-sup-run" >}}) flag. |
| `HAB_USER` | Supervisor | no default | User key to use when running with [service group encryption]({{< relref "using_habitat#using-encryption" >}}) |
