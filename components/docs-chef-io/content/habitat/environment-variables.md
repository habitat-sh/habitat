...
+++
[\[edit on GitHub\]](https://github.com/habitat-sh/habitat/blob/master/components/docs-chef-io/content/habitat/environment-variables.md)
...
| `HAB_CTL_SECRET` | Supervisor | no default | Shared secret used for [communicating with a Supervisor](/using-habitat/#remote-control). |
...
| `HAB_ORG` | Supervisor | no default | Organization to use when running with [service group encryption](/using-habitat#using-encryption)
...
| `HAB_RING` | Supervisor | no default | The name of the ring used by the Supervisor when running with [wire encryption](/using-habitat#using-encryption) |
| `HAB_RING_KEY` | Supervisor | no default | The contents of the ring key when running with [wire encryption](/using-habitat#using-encryption). Useful when running in a container. |
...
| `HAB_SUP_UPDATE_MS` | Supervisor | 60000 | Interval in milliseconds governing how often to check for Supervisor updates when running with the [--auto-update](/habitat-cli/#hab-sup-run) flag. Note: This variable has been deprecated. Users should instead utilize the [--auto-update-period](/habitat-cli/#hab-sup-run) flag. |
| `HAB_UPDATE_STRATEGY_FREQUENCY_MS` | Supervisor | 60000 | Interval in milliseconds governing how often to check for service updates when running with an [update strategy](/using-habitat#using-updates). Note: This variable has been deprecated. Users should instead utilize the [--service-update-period](/habitat-cli/#hab-sup-run) flag. |
| `HAB_USER` | Supervisor | no default | User key to use when running with [service group encryption](/using-habitat#using-encryption) |
