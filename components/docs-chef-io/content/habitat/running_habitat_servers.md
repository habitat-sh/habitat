...
First, you must [install Chef Habitat]({{< relref "install-habitat" >}}) itself on the machine.
...
As with Linux, you must first [install Chef Habitat]({{< relref "install-habitat" >}}) on the machine. Unlike Linux, however, the Windows Supervisor has no requirements for any `hab` user.
