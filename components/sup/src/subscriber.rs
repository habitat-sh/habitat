...
fn main() {
    println!(
        "Welcome to the testing subscriber for the Automate / Habitat Event Streaming Prototype"
    );
    ...
    let listener = NatsClient::from_options(options)
        .map_err(Into::into)
        .and_then(|client| {
        ...
        .and_then(|client| {
            let opts = SubscribeOptionsBuilder::default().build().unwrap();
