...
pub trait Periodic {
    ...
    fn sleep_until(&self, next_period_start: SteadyTime) {
        if let Some(time_to_wait) = Self::time_to_wait(SteadyTime::now(), next_period_start) {
    ...
    fn time_to_wait(now: SteadyTime, next_period_start: SteadyTime) -> Option<StdDuration> {
        let time_to_wait = (next_period_start - now).num_milliseconds();
        if time_to_wait > 0 {
            Some(StdDuration::from_millis(time_to_wait as u64))
        } else {
            None
        }
    }
...
mod test {
    ...
    fn time_to_wait_is_some_when_next_period_start_is_in_the_future() {
        ...
        let time_until_next_period = StdDuration::from_secs(1);
        let next_period_start = now + Duration::from_std(time_until_next_period).unwrap();
