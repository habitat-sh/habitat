...
impl Manager {
    ...
    fn add_service(&mut self, spec: ServiceSpec) {
        ...
        if let Err(_) = Self::create_svc_path(&service, &spec.ident) {
            return;
        }
    ...
    pub fn run(mut self, svc: Option<protocol::ctl::SvcLoad>) -> Result<()> {
        ...
        loop {
            ...
            self.state
                .census_ring
                .write()
                .expect("CensusRing lock is poisoned!")
                .update_from_rumors(
                ...
                for spec in specs.iter_mut() {
                    match opts.render_only {
                        Some(true) => {
                            ...
                            Self::create_svc_path(&service, &spec.ident)?;

                            service.write_templates(
                                &mgr.census_ring
                                    .read()
                                    .expect("CensusRing lock is poisoned!"),
                            );
    ...
    fn create_svc_path(service: &Service, ident: &PackageIdent) -> Result<()> {
