...
impl NatsMessageStream {
    pub async fn new(supervisor_id: &str, config: EventStreamConfig) -> Result<NatsMessageStream> {
        ...
        let nats_options  = match server_certificate {
            Some(nats_options) => {
                let cert_path: PathBuf = nats_options.into(); 
                Options::with_token(&token.to_string())
                    .with_name(&name)
        ...
        let connection : Connection = match connect_method.into() {
            Some(timeout) => {
                println!("Timeout used -> {:?}", timeout);
                time::timeout(timeout,nats_options.connect(&url.to_string()))
                    .await
                    .map_err(|_| Error::ConnectNatsServer)??
            } 
            None => {
                println!("Timeout not used");
                nats_options.connect(&url.to_string())
                    .await?
            }
        };
        ...
        tokio::spawn(async move {
            while let Some(packet) = rx.next().await {
                ...
                let res = connection.publish(&subj, packet.payload()).await;
