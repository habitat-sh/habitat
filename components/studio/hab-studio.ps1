    ...
    if($env:HAB_ORIGIN_KEYS) {
        ...
        $env:HAB_ORIGIN_KEYS.Split(" ") | ForEach-Object {
            ...
            if($LASTEXITCODE -eq 0) {
                ...
                Write-Host "you run 'hab cli setup'."
                Write-Host ""
                Write-Host "Note that if you run 'hab cli setup' in an elevated console as an"
                ...
                Write-Host "created during 'hab cli setup' in a non-elevated console is only"
        ...
        Write-Host "variable to your origin name or run 'hab cli setup' and specify a"
        Write-Host "default origin."
        Write-Host ""
        Write-Host "Note that if you ran 'hab cli setup' in an elevated console as an"
        ...
        Write-Host "specified during 'hab cli setup' in a non-elevated console is only"
        Write-Host "accesible to a non-elevated Studio."
        $env:FS_ROOT=$HAB_STUDIO_ROOT
        $env:HAB_CACHE_KEY_PATH = Join-Path $env:FS_ROOT "hab\cache\keys"
    }
...
$script:version='2.1.84'
# The author of this program
$script:author='The Habitat Maintainers <humans@habitat.sh>'
