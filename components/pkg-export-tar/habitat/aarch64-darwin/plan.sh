...
do_download() {
    ...
    "$tar_binary" -czf "$HAB_CACHE_SRC_PATH"/"$pkg_filename" components/ test-services/ Cargo.toml Cargo.lock NOTICES.txt  || exit
...
do_build() {
    ...
    cargo build "$build_type" --target="$rustc_target" --verbose --bin hab-pkg-export-tar
    popd >/dev/null || exit
}
...
# shellcheck disable=2154
do_install() {
    install -v -D "$CARGO_TARGET_DIR"/"$rustc_target"/release/hab-pkg-export-tar \
        "$pkg_prefix"/bin/hab-pkg-export-tar
    install -v -D "$SRC_PATH/../../NOTICES.txt" "$pkg_prefix"/NOTICES.txt
}
