...
fn status_value(worker: &mut Worker) -> String {
...
fn state_become_leader(worker: &mut Worker) -> BldrResult<(State, u32)> {
    ...
    let (statuscode, response) =
        etcd::set(&key, &[("value", &status_value(worker)), ("prevExist", "false")]).unwrap();
    debug!("Response is {:?} {}", statuscode, response);
    match statuscode {
        StatusCode::Created => {
        ...
        StatusCode::PreconditionFailed => {
            ...
            let (is_it_me_statuscode, is_it_me_response) =
                etcd::set(&key, &[("value", &status_value(worker)), ("prevValue", &status_value(worker))]).unwrap();
...
fn state_become_follower(worker: &mut Worker) -> BldrResult<(State, u32)> {
    ...
    let (statuscode, response) =
        etcd::set(&key, &[("value", &status_value(worker))]).unwrap();
    debug!("Response is {:?} {}", statuscode, response);
    match statuscode {
        StatusCode::Created => {
