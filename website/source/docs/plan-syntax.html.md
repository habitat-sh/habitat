...
These configuration settings are referenced using [mustache-style](https://mustache.github.io/mustache.5.html) tags. Mustache functionality such as sections can be used to enable some conditional logic on what values are used. For an example on how these settings are used in plan hooks, see [Add hooks to your plan](/tutorials/getting-started-add-hooks) in the getting started tutorial.
