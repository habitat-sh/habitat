...
fn setup_key_env(test_name: &str) -> String {
...
fn crypto_generate_key_revisions_test() {
    let key_dir = setup_key_env("key_revisions");
    let test_key_name = "habitat123";
    let crypto_ctx = hcore::crypto::Context::new(&key_dir);
...
fn crypto_generate_box_keys_test() {
    let key_dir = setup_key_env("generate_box_keys");
    let crypto_ctx = hcore::crypto::Context::new(&key_dir);
    ...
    if let None = wait_until_ok(|| crypto_ctx.generate_service_box_key(test_org, test_service_group)) {
...
fn mixed_key_revisions_test() {
    ...
    let key_dir = setup_key_env("mixed_key_revision_test");
    let crypto_ctx = hcore::crypto::Context::new(&key_dir);
    let mut revisions = Vec::new();

    for _ in 0..3 {
        match wait_until_ok(|| {
            crypto_ctx.generate_user_box_key("calculating_infinity")
        }) {
...
fn crypto_encrypt_decrypt_test() {
    ...
    // it fails.
    let key_dir = setup_key_env("encrypt_decrypt");
    let crypto_ctx = hcore::crypto::Context::new(&key_dir);
    let test_org = "someorg";
    let test_user1 = "foo1";
    ...
    if let None = wait_until_ok(|| crypto_ctx.generate_service_box_key(test_org, test_service_group1)) {
        panic!("Can't generate a service box key");
    }
