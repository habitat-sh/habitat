...
fn docker_cmd(args: &[&str]) -> Docker {
    println!("{}: Starting docker with {:?}", thread::current().name().unwrap_or("main"), args);
    let mut cmd = command::run("docker", args).unwrap_or_else(|x| panic!("{:?}", x));
    cmd.wait_with_output();
    let mut id = String::from(cmd.stdout());
    id.pop();
    println!("{}: Docker exited with: {:?}, stdout: {}, stderr: {}", thread::current().name().unwrap_or("main"), cmd.status().code(), cmd.stdout(), cmd.stderr());
    println!("{}: Docker container: {}", id, thread::current().name().unwrap_or("main"));
...
impl Docker {
    ...
    pub fn logs(&self) -> String {
        loop {
            ...
            thread::sleep_ms(500);
            let mut cmd = command::run("docker", &["logs", &self.container_id]).unwrap_or_else(|x| panic!("{:?}", x));
            cmd.wait_with_output();
            let output = String::from(cmd.stdout());
            let error = String::from(cmd.stderr());
            let re = Regex::new(r"i/o timeout").unwrap();
            if re.is_match(&output) {
                println!("{}: An i/o error failed, retrying", thread::current().name().unwrap_or("main"));
                continue;
            }
    ...
    pub fn wait_until(&self, ready_regex: &str) -> bool {
        ...
        println!("Container not ready after 5 seconds, looking for {}", ready_regex);
        false
    }

    pub fn stop(&self) {
        let mut cmd = command::run("docker", &["stop", &self.container_id]).unwrap_or_else(|x| panic!("{:?}", x));
        cmd.wait_with_output();
    }
}
    ...
    fn drop(&mut self) {
        if thread::panicking() {
            if let None = env::var_os("BLDR_DOCKER_KEEP") {
                let mut cmd = command::run("docker", &["rm", "-f", &self.container_id]).unwrap_or_else(|x| panic!("{:?}", x));
                cmd.wait_with_output();
            }
        } else {
            let mut cmd = command::run("docker", &["rm", "-f", &self.container_id]).unwrap_or_else(|x| panic!("{:?}", x));
