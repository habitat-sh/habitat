ALL_PKGS := linux-headers glibc zlib file binutils m4 gmp mpfr libmpc gcc patchelf gcc-libs bzip2 pkg-config ncurses attr acl libcap sed shadow psmisc procps-ng coreutils bison flex pcre grep readline bash bc tar gawk
BOOTSTRAP_TOOLCHAIN_PKGS := linux-headers glibc zlib file binutils m4 gmp mpfr libmpc gcc patchelf-using-bootstrap-tools gcc-libs patchelf bzip2 pkg-config ncurses attr acl libcap sed shadow psmisc procps-ng coreutils bison flex pcre grep readline bash bc tar gawk
