...
pkg_service_run="bin/bldr-vault ${pkg_svc_config_path}/app.cfg.toml"

do_build() {
  pushd $PLAN_CONTEXT/../../components/builder-vault > /dev/null
  cargo clean
...
do_install() {
  install -v -D $PLAN_CONTEXT/../../components/builder-vault/target/debug/bldr-vault $pkg_prefix/bin/bldr-vault
}
